namespace Pager
{
	...
	internal class PagerBar : UIView
	{
		public PagerBar(PagerStyle style)
		{
			...
			this.style = style;
            this.BackgroundColor = UIColor.FromRGB(246,128,25);
		...
		private void UpdateLabels()
		{
			...
			{
                var imagen = new UIImageView();
                if (i == 0)
                    imagen.Image = UIImage.FromFile("horse_icon");
                else if(i==1)
                    imagen.Image = UIImage.FromFile("timeline_icon");
                else if (i == 2)
                    imagen.Image = UIImage.FromFile("map_icon");
                else if (i == 3)
                    imagen.Image = UIImage.FromFile("bell_icon");
                else if (i == 4)
                    imagen.Image = UIImage.FromFile("menu_icon");
                ...
                imagen.Frame = new CGRect((label.Frame.Width/2)-15,(label.Frame.Height/2)-15, 30,30);
                label.Add(imagen);
				this.labels[i] = label;
